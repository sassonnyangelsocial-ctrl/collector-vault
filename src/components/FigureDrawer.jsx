...
export default function FigureDrawer({ figure, state, onClose, onSave }) {
  ...
  return <><button className="drawer-backdrop" aria-label="Close figure details" onClick={onClose} /><aside className="figure-drawer">
    ...
    <p className="drawer-series">{figure.series?.brand?.name} · {figure.series?.name}</p><div className="drawer-title-row"><h2>{figure.name}</h2><button className={`favorite-button large ${state.favorite ? 'active' : ''}`} onClick={() => onSave(figure, { favorite: !state.favorite })}>★</button></div><span className="rarity-badge">{figure.rarity}</span>
    <section className="drawer-section market-panel"><h3>Current price context</h3>{market ? <><strong className="market-price">${Number(market.estimated_value).toFixed(2)} {market.currency}</strong><p>${Number(market.low_value).toFixed(2)}–${Number(market.high_value).toFixed(2)} · {market.confidence} confidence · {market.as_of_date}</p><small>{market.methodology}</small>{market.source_urls?.length > 0 && <div className="listing-links">{market.source_urls.slice(0, 5).map((url, index) => <a key={url} href={url} target="_blank" rel="noreferrer">Source {index + 1} ↗</a>)}</div>}</> : status === 'loading' ? <><strong>Checking current listings…</strong><p>Matching this exact collectible and series against single-item listings for sale.</p></> : <><strong>No exact-price estimate available</strong><p>There were not enough exact single-item matches to calculate a defensible estimate.</p>{searchUrl && <a href={searchUrl} target="_blank" rel="noreferrer">See current matching listings ↗</a>}</>}</section>
    <section className="drawer-section provenance-panel"><h3>Verification</h3><p>{figure.image_verified_at ? `Official image verified ${new Date(figure.image_verified_at).toLocaleDateString()}.` : 'The manufacturer does not publish an individual reveal image for this item.'}</p>{figure.image_source_url && <a href={figure.image_source_url} target="_blank" rel="noreferrer">Open official {figure.series?.brand?.name || 'brand'} source ↗</a>}</section>
