...
export default function AboutPage({ session }) {
  ...
  const launchPromoOpen = Date.now() <= new Date('2026-08-08T03:59:59.999Z').getTime()
  const sessionPromoEligible = session && new Date(session.user.created_at).getTime() >= new Date('2026-08-02T04:00:00.000Z').getTime() && new Date(session.user.created_at).getTime() <= new Date('2026-08-08T03:59:59.999Z').getTime()
  return <main className="about-page">
    ...
    {launchPromoOpen&&<section className="trust-strip" aria-label="Launch week membership offer"><strong>First month FREE</strong><span>New accounts created August 2–7, 2026</span><span>Subscribe by Friday, August 7</span><button className="about-nav-cta" onClick={session?openApp:()=>signIn('signup')}>{session?(sessionPromoEligible?'Claim my free month':'View membership'):'Sign up now'}</button></section>}
    ...
    <section className="trust-strip" aria-label="Collector Vault highlights"><strong>1,000+ cataloged collectibles</strong><span>Sonny Angel · SMISKI · POP MART</span><span>Wishlist • ISO • DISO • Trades</span><span>Seller Pro</span><span>Restock alerts</span><span>Installable on phones</span></section>
