...
export default function TradeHubPage({ session }) {
  ...
  async function shareToInstagram() {
    ...
    const caption = `I’m building my collector vault with @collectorvaultapp 💗

Track your collection, ISO/DISO, and trades in one place. Create your free vault with my link:
${inviteLink}`
    try {
      await navigator.clipboard.writeText(caption)
      setNotice('Instagram caption and your personal link copied. Paste them into your Story, Reel, or caption.')
    } catch { setNotice('Copy your personal link, then add it to your Instagram Story, Reel, or bio.') }
  ...
  return <main className="trade-hub">
    ...
    <section className="subscriber-invite seller-panel"><div><span className="eyebrow">Invite friends</span><h2>Send a special signup invitation</h2><p>Share your personal link by text, email, Instagram, or Facebook.</p></div>{inviteLink ? <><div className="invite-link-row"><input aria-label="Your special Collector Vault signup link" readOnly value={inviteLink} /><button onClick={() => copyInviteLink()}>Copy link</button><button className="primary-button" onClick={shareInviteLink}>Share</button></div><div className="invite-social-row"><button type="button" onClick={shareToInstagram}>Instagram caption + link</button><button type="button" onClick={shareToFacebook}>Share on Facebook</button></div><small className="invite-social-note">Instagram opens after your caption is copied; publish only if you choose. Facebook opens its normal share dialog.</small><form className="invite-email-form" onSubmit={emailInvite}><input required type="email" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} placeholder="friend@example.com" aria-label="Friend's email address" /><button className="primary-button">Email invite</button></form></> : <button className="primary-button" onClick={createInviteLink}>Create my special link</button>}</section>
