...
export function useLiveMarket(figure) {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl(`/api/market-price?figure_id=${encodeURIComponent(figure.id)}`)).then(async response => { const body = await response.json(); if (!response.ok) throw body; setMarket(body); setStatus('ready') }).catch(error => { setSearchUrl(error.search_url || `https://www.ebay.com/sch/i.html?_nkw=${encodeURIComponent(`${figure.series?.brand?.name || ''} ${figure.series?.name || ''} ${figure.name}`)}`); setStatus('unavailable') })
  }, [figure.id])
