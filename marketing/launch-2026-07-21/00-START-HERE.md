# Collector Vault launch center

Launch date: July 21, 2026

Live product: https://collector-vault-one.vercel.app

Primary message: Collector Vault gives collectors one organized home for collections, wishlists, ISOs, DISOs, trades, source-following alerts, and seller operations. It can now be installed directly from the website on iPhone and Android without an app store.

Primary call to action: **Open Collector Vault and tap Install app.**

Use these files in order:

1. `01-LAUNCH-CHECKLIST.md` — the exact launch-day sequence.
2. `02-RECIPIENTS.md` — vetted businesses, communities, and platforms.
3. `recipient-tracker.csv` — outreach status tracker.
4. `03-EMAILS-AND-DMS.md` — personalized outreach copy.
5. `04-SOCIAL-POSTS.md` — ready-to-post captions, Reel scripts, and Stories.
6. `05-MEDIA-KIT.md` — consistent facts for press and partners.

Campaign assets are in `../assets/`.

## Required disclosure

Collector Vault is an independent collector tool. It is not affiliated with or endorsed by Sonny Angel, Dreams Inc., Whatnot, eBay, or any retailer.

Do not advertise guaranteed restocks, guaranteed pricing, guaranteed profit, or paid raffles/waffles. Use “estimated market price” only after the live pricing feed is operational and verified. Describe the wheel only as a free giveaway-selection tool.
